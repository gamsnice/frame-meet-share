...
export default function FAQSection() {
  const faqs = [{
    ...
    answer: "Any event where networking and social sharing matter: conferences, summits, festivals, corporate events, meetups, university events, and more.",
    category: "Use cases"
  }, {
    ...
    answer: "Yes! The free plan includes 1 event. Pro plans (coming soon) support unlimited events with additional features.",
    category: "Plans"
  }];
  ...
  return <section className="py-20 bg-muted/10 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-4xl relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-8">
          ...
          setShowAll(false); // reset to hottest 3 when switching category
                ...
                ${activeCategory === cat ? "bg-primary/20 border-primary/60 text-primary-foreground" : "bg-background/40 border-border/60 text-muted-foreground hover:border-primary/40 hover:text-foreground"}`}>
              {cat}
            </button>)}
        </div>
        ...
        <Card className="bg-background/60 border-border/60 shadow-md shadow-black/20">
          <Accordion type="single" collapsible className="divide-y divide-border/40">
            {visibleFaqs.map((faq, index) => <AccordionItem key={`${faq.question}-${index}`} value={`item-${index}`} className="border-none px-4 md:px-6">
          ...
          {filteredFaqs.length > 3 && <div className="flex justify-center pb-4 pt-2">
              <button type="button" onClick={() => setShowAll(prev => !prev)} className="text-sm md:text-base px-4 py-2 rounded-full border border-border/60 hover:border-primary/60 hover:text-primary transition-colors">
                {showAll ? "Show fewer questions" : hiddenCount > 0 ? `Show all ${filteredFaqs.length} questions` : "Show all questions"}
              </button>
            </div>}
        </Card>
      </div>
